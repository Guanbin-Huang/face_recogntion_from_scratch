...
class Detector:
    ...
    def __call__(self, orgimg):
        # Load model
        img_size = 800
        conf_thres = 0.3
        iou_thres = 0.5

        # images = cv2.imread(orgimg)
        ...
        if r != 1:  # always resize down, only resize up if training with augmentation
            ...
            img0 = cv2.resize(img0, (int(w0 * r), int(h0 * r)), interpolation=interp)  # 将图片resize为最大边为img_size大小

        imgsz = check_img_size(img_size, s=self.model.stride.max())  # check img_size 使图片最大边可以被最大stride整除
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for j in range(det.size()[0]):
                    ...
                    bbox_list = show_results(orgimg, xywh, conf, landmarks, j)
                    bboxes.append(bbox_list)
            else:
                bboxes = None
                # bboxes = numpy.zeros((0, 10), dtype=int).tolist()
        return bboxes

if __name__ == "__main__":
    detector = Detector(r"D:\pycharmproject\FaceRecognizition\yolov5_face_master\runs\train\exp5\weights\best.pt")

    path = r"D:\pycharmproject\FaceRecognizition\yolov5_face_master\show_image.jpg"
    detector(path)
